...
proxy_attrs = ['_obj', '_attr_call_log']
...
class Proxy:
    ...
    def was_called(self, attr):
        return attr in self._attr_call_log

    def number_of_times_called(self, attr):
        return sum([1 for i in self._attr_call_log if i == attr])

    def messages(self):
        return self._attr_call_log

    def __getattr__(self, item):
        self._attr_call_log.append(item)
        return self._obj.__getattribute__(item)

    def __setattr__(self, key, value):
        if key not in proxy_attrs:
            self._attr_call_log.append(key)
